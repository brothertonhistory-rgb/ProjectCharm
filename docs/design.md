...
- **The opening five is the earliest five acquired that seats a playable floor: at least 1 big and at least 2 guards** (Session 29.1, amending the S29 first-five rule). `BuildOpeningFive` is a greedy walk of the acquisition order with the draft's last-slot feasibility shape — take the earliest player, skipping one only when taking him would leave too few remaining slots to cover the unmet quota; when the raw first five already satisfies the quotas, the opening five IS the raw first five (~38% of stock rosters at the fixed seed). Every roster is exactly 4G/3W/3B so the quota is always satisfiable. Deterministic; rank-blind by signature (inputs: acquisition order + positions, never rank or ratings). **Accepted residual, ruled and recorded:** a no-wing five is possible (small-ball), and benched wings then cannot check in under the fatigue fence's same-position pairing — the smoke sim prints one warning line per affected side, never silent. Acquisition order is immutable and is the printed depth order.
...
- **"Some elite opening fives carry no big" (S29) — RESOLVED by the 29.1 seating floor** (≥1 B / ≥2 G). The remaining, accepted residual is the no-wing five (small-ball), whose benched wings cannot enter under the same-position fence — warned on the smoke-sim page, one line per affected side. Opening-five logic beyond the floor still belongs to a future lineup-or-coaching layer.
...
Session 30 extracted the committed `RunGenMatchup` loop body into **`RunSingleGenGame`** (configs pre-loaded once per caller into a `GenEngineConfigs` record — same Load calls, same order; seat, depth charts, fence policy, resolver, governor with the substitution seam, run, attribute on the resolver seed). `gen` and the smoke sim compute their original seeds and side alternation and delegate — byte-for-byte by construction. The season calls the same helper per game: per-school `GenPlayerRow` lists built **once** (the smoke sim's exact Rows shape — acquisition order, the 29.1 seating floor, one absent-position warning per affected school), sides stamped **per game** (home rows → PlayerIds 1–10, away → 11–20), **HomeSchool → the engine's Home side, full stop** (`GameState.HomeScore` credits the home school; the §1b mapping, proven end to end by Phase 55's independent replay because a flipped mapping passes every conservation and determinism check).
...
Identity block (world, seed, **schedule fingerprint**, the schedule contract line), the divvy regeneration with prep warnings, a progress line every 500 games, then: **(i) the full ranked table** — every school by W-L, ties by school id; **(ii) the proof table** — average wins per prestige band (the divvy's five bands); **(iii) the escapes** — top ten by wins over own-band average, each with its leaked top-decile players named (pool id, role, rank, tier) or "no top-decile players" when the overperformance had no star behind it; **(iv) the sanity pulse** — OT games/periods, average total score, and a loud anomaly line if a tie ever survives. The proof table and escapes are **read-and-report** — findings for basketball judgment, never assertions, never seed-shopped. First stock reading (seed 20260703), recorded: band averages **12.9 / 14.5 / 14.5 / 16.5 / 17.6** — the climb is real, the middle plateau and the ~5-win top-to-bottom spread are calibration data points; OT 167/5205 (3.2%); average total 150.5; elite variance wide (Duke 98 → 15-15, Kansas 97 → 22-8); the no-wing residual shows mixed evidence at the extremes — no clean signal claimed.
