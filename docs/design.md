...
Leg→rating ownership: **size** owns Height, Wingspan, Weight, and — deliberately — OffensiveRebounding/DefensiveRebounding (see below); **athleticism** owns the engine's composite five (Strength, Speed, Quickness, FirstStep, Vertical) plus Endurance and Hustle; **skill** owns everything else (shooting, ball, defense, mental). Tendencies come from the **role**, not a leg (and must satisfy the engine's nonzero-sum rule).
