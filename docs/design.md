...
`dotnet run -- gen [gen.json]` (mirrors `bench`; returns before the validation suite). `gen.json` defines two programs — each a `prestige` plus an optional `lean` — plus `gameCount`, `baseSeed`, and a `genSeed` (roster generation is reproducible from it). The reader is the **same strict tree-walk** the bench uses (rejects unknown/duplicate keys, out-of-range prestige, unknown lean, at every level). Flow: generate both rosters → print the **roster sheet** (ten rows/program: leg symbols, per-leg strength, FT, depth, labeled by roster slot) → seat each program's five starters, stamp them A → 1–5 / B → 6–10 via the existing `StampPlayerId` seam (constructing stamped copies, never mutating the generated players, never using a slot number as a `PlayerId`) → run `RunBenchMatchup` + `PrintBenchChannels` + `PrintBenchBoxScore` **verbatim** (their `[A]/[B] Slot 1–5` labels now denote the starter cohort, distinct from the ten-slot sheet). Each player passes `Player.Validate()` as a post-construction assertion; a failure is a loud generation bug before any sim.
