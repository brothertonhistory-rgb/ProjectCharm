...
**The readout — team AND per-player.** Per-player credit is read from the same engine-stamped attribution the box score uses (`AttributeGame` → per-slot `OReb`/`DReb`), so the CSV shows whether the swept player personally gets credited or the engine routes the board elsewhere — the team rate alone cannot answer that. Team ORB rate uses the real `OrbWon/OrbChances` denominator off the possession records; per-player DRB has no chances denominator, so per-player columns are boards-per-game counts. Team totals (summed per-player attribution) and the records-side `OrbWon` are the same boards counted two ways — an internal consistency check. The CSV is `attribute_{outputName}_sweep.csv` beside the binary (the SizeExperiment convention); the console table prints the swept slot's own ORB/DRB alongside team totals and the rebound margin, with `FLAT_50_CONTROL`/`BODY50_REB50`-style rows as built-in neutrality anchors.

**Proven on rebounding (S45), and what it found.** The first family run (OffReb/DefReb isolated sweeps + an eight-row body×rating interaction block) confirmed the instrument (control at zero margin, the flat anchor identical to control, each rating isolated, curves monotone) and produced the layer's first finding: **rebounding is a rating-gated skill the body amplifies but does not grant** — a freak body with a zero rebounding rating grabs the same ≈0.2 boards a tiny weakling does, because the individual rebounder picker makes the body a multiplier on the rating (floored to 1 at rating 0) rather than a standalone term. Full finding, the two-step team-vs-picker model, and the cross-selector audit live in `docs/attribute-meaning.md`. The `sweep` bench is the template every later family session aims: each rating swept in isolation at 5-point steps, plus an interaction block where the rating plausibly fights another attribute.
