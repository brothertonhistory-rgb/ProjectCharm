...
**The isolation discipline (Emmett's ruling).** Every dial except the one under study is pinned to the flat all-50 baseline — the swept player's other ratings, all four teammates, and the entire opponent. No random-rolled context. The curve is therefore the swept rating's **pure, exactly-repeatable** signal, run-to-run; random context would reintroduce the noise the flat bench exists to remove. The swept player is a single slot (default 5, the C) so the readout answers "does *he* rebound?" rather than a team effect.
