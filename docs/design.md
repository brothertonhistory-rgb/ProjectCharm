...
The arc's second pass: a full season played through the real engine on divvied rosters. **Harness-only** — `src/Charm.Harness/Program.Season.cs` plus Phase 55, nothing under `src/Charm.Engine/` — and the `gen` mode and divvy smoke sim stay byte-for-byte behind the single-game extraction (below). CLI: `season <world.json> <seed>` (returns before the suite). Everything a season needs is a pure function of (world, seed, config): rosters regenerate from the S29 divvy, the schedule from its own RNG stream, per-game engine seeds from a stride-2 fan-out — nothing persisted, nothing cached across runs.

### The schedule contract (mirrored bit-for-bit by `tools/schedule_oracle.py` — the oracle docstring is the authoritative spec)

Every team plays **30 games: 16 conference + 14 non-conference**, neutral floors (the road seam is 0 this pass), **exactly 15 home / 15 away**.

- **RNG:** one SplitMix64 stream, `WorldRng(seasonSeed ^ 0x5EA5C4ED)` (the sample-sheet XOR pattern — its own stream, decoupled from the divvy). Consumption order per construction attempt: one Fisher-Yates ring shuffle (n−1 draws), then one draw per actual conflict repair (the scan start offset). **Conference slates and orientation consume no randomness.**
- **Conference slates (no RNG):** conferences by id, members by school id indexed 0..s−1; `base = 16/(s−1)` meetings per pair; the remainder `r = 16 − base·(s−1)` distributed as an r-regular **canonical circulant** on member indices (r even → offsets 1..r/2; r odd — s even, guaranteed by parity — → offsets 1..(r−1)/2 plus the diameter matching). Every pair meets floor or ceil of 16/(s−1) times. The r·s parity preflight can provably never fire at 16 games (s odd forces r even); it stands as contract protection for a future games-count change.
- **Non-conference (RNG):** a **14-regular simple graph, no conference-mates**: shuffle schools into a ring, take the k=1..7 circulant (14-regular by construction), collect mate conflicts in scan order, repair each by a **double-edge swap** (seeded scan start; first legal of R1 `(a,c)+(b,d)` then R2 `(a,d)+(b,c)`; both new pairs must be non-mates and absent). Swaps preserve regularity, so only simplicity and the mate ban are ever at stake. A stuck conflict fails the **attempt**; construction restarts with a fresh shuffle from the same continuing stream, up to 20 attempts, then fails loudly naming the pairing. Measured: stock completes on attempt 1 (260/260 builds); the razor-tight fixture (15 eligible, 14 needed) retries on ~1 in 20 seeds, never past attempt 3 in 700 builds.
- **Preflight (necessary conditions, named failures):** a one-school conference (`s−1 = 0`), the r·s parity, and non-conference eligibility (`n − s ≥ 14` per school) each fail loudly naming the school/conference. A two-school conference is legal (base 16, r 0). The construction itself is the final attempt — a per-seed build failure is loud and proves *this builder* produced no legal slate, never that none exists.
- **Orientation (no RNG):** the full multigraph has every degree 30 (even), so every component carries an Eulerian circuit; iterative Hierholzer (components by lowest school id, per-vertex adjacency in game-index order) orients each game in its consumption direction, from = home — a closed circuit forces out = in = **exactly 15/15** at every vertex.
- **Fingerprint:** `gameIndex|kind|homeId|awayId` per line in schedule order (never re-sorted), UTF-8, SHA-256 lowercase hex — printed on the page, asserted by Phase 55 against the oracle export at the fixed seed.
...
Identity block (world, seed, **schedule fingerprint**, the schedule contract line), the divvy regeneration with prep warnings, a progress line every 500 games, then: **(i) the full ranked table** — every school by W-L, ties by school id; **(ii) the proof table** — average wins per prestige band (the divvy's five bands); **(iii) the escapes** — top ten by wins over own-band average, each with its leaked top-decile players named (pool id, role, rank, tier) or "no top-decile players" when the overperformance had no star behind it; **(iv) the sanity pulse** — OT games/periods, average total score, and a loud anomaly line if a tie ever survives; **(v) the calibration readout** (Session 31) — sim vs the D1 decade blend line by line with a LOW/OK/HIGH verdict per row, plus the seconds-per-possession split by ending (see the calibration-instrument section below). The proof table, escapes, and calibration readout are **read-and-report** — findings for basketball judgment, never assertions, never seed-shopped. Current stock reading (seed 20260703, post-30.1 seating floor): band averages **12.8 / 14.4 / 14.8 / 16.5 / 17.5** — cleanly ordered bottom to top (the S30 middle-band plateau resolved once every school’s wings played; one-seed observation, not a proven causal claim — the S30 reading is preserved in the journal); the ~5-win top-to-bottom spread stands as a calibration data point; OT 152/5205 (2.9%); average total 150.5; elite variance wide (Duke 98 → 16-14, Kansas 97 → 22-8, La Salle 48 → 25-5 atop the table behind three leaked top-decile players).

### Phase 55 (suite; the season CLI returns before it)

Stock schedule legality in full (counts, 15/15, mate ban, repeat ban, floor/ceil meetings, 5205 total) plus the **oracle fingerprint** and the seed-set disjointness; build determinism; the preflight rejecting a rigged one-school conference **by name** — the rigged world first proven valid under the Pass 1 validator, so the red is provably the preflight's — while explicitly not rejecting a two-school conference; season prep for all 347 (world unmutated, both prestige-0 schools still read 0 and seat a legal five, the no-GenProgram fact asserted as prose in the check name); a full fixture season **conserving exactly** (every team 30 results, wins == losses == 300, zero ties, strict winners); the **attribution replay** (game 0 rebuilt independently, rerun on the season's seeds, scores matching the recorded result); standings asserted to be exactly the fold of the recorded results; and a second full fixture season identical score-for-score. Session 31 added **§3.8** — five machinery checks on the calibration accumulator the §3.5 fixture run now carries (conservation only, zero basketball values, zero additional games; see the calibration-instrument section below). Cost: ~600 fixture engine games of suite time, accepted.

### What a green Phase 55 does and does not prove

**Proven:** schedule legality and oracle fidelity at the fixed seed, builder determinism, full-season determinism, result conservation, and the home-credit mapping. **Not proven:** outcome realism, calibration direction, or that the proof table's shape is basketball truth. Game *outcomes* are not oracle-mirrorable (SystemRng, no .NET in the sandbox) — they are proven by the harness invariants above, which is the honest wall.

### Deferred out of this pass

Home-court advantage (the road seam exists and ships 0 — turning it on is a calibration act); real scheduling texture (dates, travel, rivalries, buy games, conference tournaments — this schedule is a placeholder graph, not a calendar); the postseason (Pass 2 ends at the regular-season table); persistence of results (a season is recomputed, never saved — the save-format discipline arrives with the career layer per the standing note); the prestige dynamics reading these standings (Pass 3, next per the arc map); and calibration tuning itself — whose measuring stick Session 31 built (the section below) and whose standing precondition, a living varied population, this pass satisfies. **Per-player season statistics were deferred out of this pass and SHIPPED IN S77** — see "The season stat layer" below; what remains deferred there is per-GAME retention (game logs, splits, streaks).
