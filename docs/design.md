...
**Final curve parameters.** Floors, ceilings, and K are the Session 50 fit and have not moved. The five **midpoints were recentered by the Session 32 make dial** (see the make-dial section at the end of this document) so that an average shooter *in the divvied league* — not authored rating 50 — lands on the observed anchors; "rating 50 = real D1 average" describes the pre-32 centering only. As of Session 32 **all 20 parameters live in config.json's RollH section, which is the canonical live calibration** (the identical `RollHConfig` class defaults are the fallback for a config missing the keys):

| Zone | Floor | Ceiling | K | Midpoint (S32) |
|------|-------|---------|------|----------|
| Three | 0.1608 | 0.6328 | 0.029646 | 109.9675 |
| Long  | 0.1934 | 0.6034 | 0.034190 | 102.6819 |
...
`rank = L1 + 0.9·L2 + 0.4·L3 + 0.045·max(0, L3−30)²` over the three leg means, sorted descending. **Every leg is graded on the position's own scale (Session 29.1 — scouting is position-relative, and shooting must not buy access):** the SIZE leg is normalized by a single per-position affine map anchored (ordinary-lo → 44, plus-hi → 88), slope 44/24 both directions (forward places the SIZE gradient band; inverse feeds the rank); the SKILL mean excludes the position's permitted holes **and FreeThrow** (the FT draw and the sheet's FT column are untouched — FT just buys no prestige access); a big's ATH mean re-adds `GenBigAthDownshift`, the exact inverse of the generation-side −8 on the pool path (proven post-enforcement: the shift is applied at all three draw sites, no floor or clamp or lean touches a big's ATH after it, and the worst post-shift draw of 26 clears both the zero-clamp and the pool leg-health floor — **standing condition: re-run this proof if the scarce band's floor or the pool leg-health floor ever moves**). Properties, oracle-pinned under the 29.1 rulings: **monotone** in every feeding rating (FT now moves it exactly zero); **convex in the third leg** (per-point deltas 1.255 → 2.245 → 3.37 across the bands); group means **strictly ordered three > borderline > useful > scarce** with one-leg below useful (stock: 260 > 219 > 183 > 164, one-leg 164.0) — **scarce and one-leg sit even by ruling** (two strong legs + one bad leg ≈ one strong leg + two passable ones; future coach layers bend individual boards toward their own preferences); true-rank overlap is robust only low on the board (useful/scarce, scarce/one-leg), while **the borderline/useful tiers separate in truth and blur through scouting error** — the board-noise sigma (~10.7) exceeds their gap (~7.5), asserted, so schools still confuse them even though a perfect scout would not (Emmett's ruling: the mixing coming entirely from scouting error is the design). Rank-vs-actual-value disagreement is the recruiting miss, by design. **Quarantine:** the rank lives on the pool row only — never on a `Player`, never printed as an overall, never sorting anything outside the draft board (roster sheets print leg means, not rank).
...
- **The opening five is the earliest five acquired that seats a playable floor: at least 1 big, at least 2 guards, and at least 1 wing** (Session 29.1 set the 1B/2G floor; Session 30.1 extended it to the wing after the first stock season showed 44 schools whose benched wings played zero possessions under the same-position fence). `BuildOpeningFive` is a greedy walk of the acquisition order with the draft’s last-slot feasibility shape — take the earliest player, skipping one only when taking him would leave too few remaining slots to cover the unmet quota; when the raw first five already satisfies the quotas, the opening five IS the raw first five. Every roster is exactly 4G/3W/3B so the quota is always satisfiable — proven exhaustively over all 4,200 distinct orderings of a 4G/3W/3B roster (Session 30.1 pre-check), asserted on live drafts by Phase 54. Deterministic; rank-blind by signature (inputs: acquisition order + positions, never rank or ratings). Every position now seats at least one starter, so every benched player has a same-position door under the fence; the absent-position warnings stay as never-fire sentinels — a line printing is a seating bug. Acquisition order is immutable and is the printed depth order.
- **Rosters are not persisted:** deterministic from world + seed; the season pass regenerates (exercised for real by Session 30's season loop — 347 rosters rebuilt from (world, seed) every run).
...
**Phase 54** (suite; the CLI returns before it): determinism at both scales; noise triples to 1e-15 plus read-order independence; every roster legal at both n; pool shape equal to the apportionment constants; a rigged pool rejected loudly naming the shortfall; rank formula fixtures to 1e-9, convexity, monotonicity spot check; the 29.1 ordering/overlap block (three > borderline > useful > scarce strict, one-leg below useful, scarce vs one-leg deliberately unordered, the two robust true-rank overlaps, the borderline/useful gap under the board-noise sigma); fixed-seed access sanity (top-decile prestige mean drafted rank above bottom-decile, ≥1 leak — robust even to oracle/C# stream divergence, since at stock scale essentially every seed leaks); protected-supply slack non-negative across both worlds plus five fixture seeds; and the **29.1 section 9** — FT-blindness (rank bit-identical at FT=25 vs FT=95), three full-pipeline per-position rank fixtures through `DivvyScoutRank` itself (closing the trap where the formula fixtures feed leg means directly and would stay green without the wiring), post access (top-decile mean drafted-BIG rank above bottom decile), and the seating floor (≥1 B / ≥2 G everywhere, deterministic, equals the raw first five wherever raw already qualifies). The suite's stock world comes from `ConvertWorld` on the csvs beside the binary (the Phase 53 path — no csproj change).
...
- **"Bigs go late everywhere" (S29) — RESOLVED by Session 29.1.** The observation was not scouting realism; it was the rank reading a big's athleticism on the raw scale, docking every big the built-in mobility downshift (~a full noise band). With athleticism graded per position, bigs sit alongside guards and wings inside every tier (mean rise +13.5, the worst-graded athletic-poor bigs +25 to +35), elite posts land on elite boards (fixed seed: top-prestige-decile mean drafted-big rank 182.3 vs 154.0; 35 of 57 blue-chip bigs on top-quartile rosters), and elite sheets open with bigs in the early acquisitions.
- **"Some elite opening fives carry no big" (S29) — RESOLVED by the 29.1 seating floor; the successor no-wing residual RETIRED by the 30.1 floor** (>=1 B / >=2 G / >=1 W). The residual’s true cost only became visible at season scale — 44 stock schools whose three wings never played a possession — and Emmett ruled the floor extends. Opening-five logic beyond the floor still belongs to a future lineup-or-coaching layer.

### Deferred out of this pass (per the brief, plus 29.1 additions)

Classifications/ages and multi-class pools; real recruiting (the divvy is its bootstrap stand-in, not its design); coaching styles and preference-driven picking (Emmett's 29.1 rulings pre-name two of its jobs: per-coach board preferences over even prospects, and lineup shape beyond the seating floor); **versatility / multi-position players** (29.1 — a second seatable position, cross-position fence pairing, coverage counting either quota, coach valuation; home: player-generation pass 2 plus the lineup/coaching layer); **the FT-hitch archetype** (29.1 — the rare broken-stroke skilled player shooting in the 40s; texture, judged after seasons produce FT volume); transfers; per-conference or regional draft biases; engine-side migration of generation; substitution policy beyond the flat fence; and any calibration of pool magnitudes beyond "looks like college basketball on the sheet."
...
# The make dial — per-zone midpoints recentered to the living population (Session 32, 2026-07-03)

The first tuning dial turned against the Session 31 instrument. **Config-only** (`config.json` RollH section) plus the instrument's per-zone rider and the committed oracle; no engine source file changed.

### What the dial is

Each zone's bounded logistic (`makePct = floor + (ceiling − floor)/(1 + exp(−K·(rating − midpoint)))`) has exactly one recentering knob: the **midpoint** — the effective rating at which a shooter sits halfway between the zone's floor and ceiling. Session 50 centered the curves at authored rating 50; the divvied population's effective shooting sits far above 50 (matchup shifts are uncapped by design, and the pre-32 card implied a league-average effective Three rating near 94), so the whole league over-shot every zone. The recentering slides each midpoint so an **average shooter in our league** lands on the observed anchors (Three 34 / Long 36 / Mid 39 / Short 43 / Rim 61). **Floors, ceilings, and K never move** — Emmett's ruling, recenter never compress: skill separation is a different dial (deferred, with its own design conversation before any K change).

### Where the numbers live

As of this session, **all 20 per-zone logistic parameters are canonicalized in config.json** — the single live home of the curve; the identical `RollHConfig` defaults are only the missing-key fallback. Live midpoints: **Three 109.9675, Long 102.6819, Mid 90.3196, Short 75.9276, Rim 68.7559.** The commit range for a proposed midpoint is **[0, 120]** (Emmett's ruling): the bound is the *effective*-rating domain, not the authored 0–99 range — Long and Three legitimately sit above 100. A proposal outside [0, 120], or an inversion demanding a clean make at/outside the curve's open (floor, ceiling) interval, is a STOP-and-report result, never a value to commit. Standing coupling (inherited from Session 50, now owned here): **if the Roll H block/foul baselines ever move, the rim/short midpoints re-derive.** Known stale-in-place: the `RollHConfig` anchor comment still says "rating 50 = real D1 average" — pre-32 centering; the one-line correction rides the next session that legitimately touches engine files.

### The oracle (`tools/make_midpoint_oracle.py`)
