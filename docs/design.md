...
Five checks on the accumulator the §3.5 fixture run carries (zero additional games), asserting **zero basketball values**: (a) triple point reconciliation — recorded game scores == sum of record Points == accumulator points, all three printed; (b) the drift guard — every record carrying turnover metadata satisfies the classifier, so a future TO label turns red instead of leaking into OTHER; (c) the bucket conservation identity — made + FT-trip + miss→DREB + miss-OOB + turnover + other + excluded == total records; (d) the per-game elapsed guard — |sum of record Elapsed − TotalSeconds| within 1e-6 in **every** game (per-game, not a season aggregate offsetting errors could satisfy; both sides sum the identical applied doubles, so exact equality is expected); (e) FGM ≥ 3PM, FTA ≥ FTM, all totals non-negative. Every league line is side-symmetric (summed over all 20 box indices), so a green §3.8 must never be read as re-proving the home-credit mapping — Phase 55's independent replay owns that.

### First stock reading (seed 20260703 — recorded for basketball judgment, not acted on)

**The headline: shots go in far too often** — FG% 55.8 vs 44.0, 3P% 50.1 vs 34.0 — and most of the card is downstream of that one fact: FGA LOW and credited rebounds very LOW because makes end possessions and misses create boards; assists HIGH (19.9 vs 13.5) partly on make volume; points HIGH (75.3) on efficiency alone. **The telling OK: ORB% 29.0 vs 28.5** — when a miss happens, the board splits realistically; the rebound machinery looks right and is merely starved of misses. Independent of the make problem: **the shot diet is three-starved** (3PA rate 0.21 vs 0.39 — a diet issue, separate from accuracy) and **the turnover composition leans dead-ball** (steals LOW 4.4 vs 6.2, total turnovers slightly HIGH). Pace, FT%, and TO% read OK. The split: turnover possessions 17.3s vs ~18.2s for shooting endings — nearly identical, the gap the deferred turnover-clock dial exists to widen. Known lines confirmed: points HIGH, OT LOW (2.9%). **Flagged, not chased: the OTHER bucket is ~4% of all possessions** (28,746 stock; 4.2% fixture — consistent) against a near-zero expectation; a per-label breakdown is a cheap instrument follow-up, and 4% is large enough to matter for the rebound and FT lines.
