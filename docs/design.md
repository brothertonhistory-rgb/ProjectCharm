...
### Shot tendencies — derived from the player's own skills, not a role table (Session 34; retuned to oracle v2, Session 35)
...
The derivation is a C# port of a committed, locked Python oracle (`tools/tendency_oracle.py`, LOCKED SPEC ORACLE v2 — the modern-era retune) — the same oracle-first discipline the rest of generation follows. The oracle is authoritative: the port mirrors it stage-for-stage and constant-for-constant, and future tuning happens in the oracle first, never in the C# alone. Four rulings shape v2: **rating drives efficiency and usage, not frequency** (a below-average perimeter shooter still fires broadly — the rating punishes him at the make curve and the volume layer, not the diet); **nonzero rating = capable** (any player with Outside > 0 launches his occasional wide-open three); **the era profile** (one explicit stage of shot-selection culture, below); and the **embraced paradox** that a poor shooter with no other tools correctly skews three-heavy, because it is all he can do (his level fit is the divvy's job, not the derivation's). The shape, in plain terms:

- **Raw per-zone capability signals** from the skills that actually earn a shot in each zone — rim from finishing plus self-created downhill drives; short from a real post touch or a floater game; mid from a credible pull-up or catch-and-shoot jumper; long two from two independent gated paths (a star's pull-up creation, or a stretch big's frontcourt-body plausibility × catch-and-shoot credibility — with **no shooting in the plausibility factor and all shooting in the credibility factor**, a clean input partition so a big earns his pop from being a big who can shoot, not from a role label). The **three signal is a two-path blend** keyed by *perimeter-ness* (small body OR a real handle): a COMPRESSED perimeter path whose frequency is broad and only weakly rating-dependent, blended with a stretch-gated interior path so a big earns arc volume only through a real stretch rating. Perimeter-ness is one shared helper, read by both this signal and the opportunity floor.
...
- **Era profile** — five weight-space multipliers applied immediately after peakedness (Rim 1.00 / Short 0.66 / Mid 0.44 / Long 0.63 / Three 2.44), encoding the modern shot-selection *culture* (threes up, mid-range way down, floaters and long twos trimmed, rim untouched) cleanly separated from individual *capability*. An earlier-era league is these five numbers and nothing else. The **Session 39** volume retune raised the three multiplier and deepened the mid/long donors (mid-range the primary donor, rim protected but not invariant), lifting the population three diagnostic from ~35.5% to ~39.0% to move the realized league 3PA rate onto its 0.39 target — a pure *selection* change, no make-curve touch. The oracle's structural gate guards the multi-level scorer's *identity* (bounded three, protected rim, live mid-range, not three-lopsided, flatter than a pure shooter) rather than a now-stale absolute three cap; and the oracle's golden-fixture emit resolves its output path beside the oracle file, so regenerating from any working directory always overwrites the committed fixture.
...
- **Opportunity floor** hands everyone the inside shots basketball hands everyone, gives a perimeter long-two floor to players who operate out there, and gives a **universal three floor to every player with any outside rating at all** (a rating-3 big fires his rare wide-open one), with a perimeter extra on top. Only a literal Outside == 0 player still reads a zero three tendency; the sliver of career buzzer heave below that is Roll G's at pie time, if ever.
