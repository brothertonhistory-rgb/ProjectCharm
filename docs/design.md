...
# Roster genesis — the national pool and the prestige-weighted divvy (Pass 1.5 → the S63 generation bridge)

Governing records: `docs/roster-genesis-brief.md` (design conversation of 2026-07-02, shipped Session 29) and the Session 63 generation bridge (Emmett rulings 2026-07-20, shipped 2026-07-21), which retired the authored pool and pointed the divvy at the real Pass-2 skill-first cohort.

**The frame.** Every school in a world file gets a ten-man roster from **one national talent pool** (10 × school count — a bootstrap standing in for four absorbed recruiting classes; classes/ages are recruiting-arc design) divided by **prestige-weighted selection**. Prestige is **access**, never quality. Talent is zero-sum: elite players are scarce because the pool says how many exist. Since Session 63 the pool is no longer an authored distribution — it is **the real Pass-2 cohort** drawn from the oracle-locked skill-first generator, so the world's talent shape is whatever the generator honestly draws. The Session 26 `gen` mode and its machinery survive untouched as a lab instrument behind their byte-for-byte wall; the prestige→depth relationship still emerges from access (S63 page, top-third-rank per roster by band: 0.41 / 1.55 / 3.65 / 5.53 / 7.60 bottom to top, every adjacent band overlapping).

### The pool (Session 63 — the Pass-2 cohort, bridged)

- **Source:** `PlayerGenPass2Live.BuildCohort(divvySeed ^ DivvyCohortSeedXor, 10 × schoolCount)` — the S44 live generator on its own derived stream (the schedule's committed XOR pattern), so the pool stays a pure function of (world, seed). The transform is oracle-locked; the bridge draws, never edits (Phase 59/60 re-prove parity every run).
...
The pool draws on its own derived stream (`BuildCohort` seeded `divvySeed ^ DivvyCohortSeedXor`; the 40-slot per-player draw order is the Phase-59 contract). The sequential `WorldRng(divvySeed)` feeds **only Phase D** (one winner draw per pick) — the S29 Phases A–C are retired with the old builder. The board noise rides its own per-pair streams, random-access by design. Reproducibility is guarded by Phase 54's byte-identical-pool determinism check (every position, role, rating, orientation, and rank identical under the same seed), not by a Python pool mirror.
