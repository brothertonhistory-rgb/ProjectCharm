...
### Shot tendencies — derived from the player's own skills, not a role table (Session 34)
...
- **Opportunity floor** hands everyone the inside shots basketball hands everyone, and gives perimeter floors *only* to players who operate out there — so a paint big who never steps to the arc still takes **zero** threes at generation. (The emergency career heave — a true non-shooter's ~0.2% buzzer three — is deferred to Roll G as a runtime pie floor, below integer-tendency resolution; it is not authored here.)
...
The derivation runs at **both** generation call sites — the roster path and the divvy pool path — after every rating mutator has finished (lean tilt, position floors, leg-health lift, and the pool's third-leg redraw all raise ratings that feed the derivation), and before the player is built. Deriving any earlier would stamp a diet from a half-finished player. A **golden-vector parity check** (17 named rating maps with exact expected diets, including rounding/tie traps) runs at the start of the `gen` command and fails loud on any C#-vs-oracle disagreement; it validates the fixture contract first (zone order, input completeness, valid diets) so a stale fixture is rejected rather than silently trusted.
