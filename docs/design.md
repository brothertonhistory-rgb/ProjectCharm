...
**(a) Leaderboards**, top ten each. Counting categories (points, rebounds, assists, steals, blocks, minutes) as **per-game averages**, so they read like a media guide. Shooting categories (FG%, 3P%, FT%) additionally print **makes-attempts, attempts per game, games played and minutes** — a percentage is neither a total nor an average, and a bare `61.2%` is not inspectable. **(b) One team in full** — all thirteen men of school id 200 (Oklahoma State) in rotation order, the view that makes a rotation legible at a glance. **(c) League distributions** — the value at rank 1/10/50/100/500/1000 and the median across all player-seasons, because distribution *shape* is where implausibility shows up first.
...
### Foul columns are labelled, because they are allocations
