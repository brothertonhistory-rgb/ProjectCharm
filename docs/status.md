...
- **Attribute-meaning layer — the `sweep` bench is BUILT (S45), its picker bug FIXED (S46), and its
  ...
  what-50-means note) live in `docs/attribute-meaning.md`, Family D. The remaining families, each
  following the same template: **A. Body** (Height, Wingspan,
  Weight) · **C. Athleticism** (7 ratings) · **E. Perimeter creation** (BallHandling, Passing,
  Playmaking, SelfCreation, OffBallMovement) — **the recommended next (S49)**: SelfCreation was
  explicitly deferred to it at the S48 gate, and the usage/creation channel the Scoring measurement
  surfaced is this family's home turf · **F. Interior offense** (PostMoves, Screening) ·
