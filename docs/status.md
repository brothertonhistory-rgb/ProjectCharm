...
- **Attribute-meaning layer — the `sweep` bench is BUILT (S45), its picker bug FIXED (S46), and its
  ...
  **height-over-defender make term the item that most gates the height→skill lean sizing**. **That term
  ...
  design conversation), (2) ~~the unforced-turnover channel (S49)~~ — **SHIPPED S56** (handling now moves the neutral turnover base; flattens the inversion, full flip deferred — see below), (3) the no-post-hunt diet gap (S51),
