...
- **★ NEXT SESSION — the USAGE↔EFFICIENCY CURVE (S59.2 ruling: the CURVE ONLY, not the dial).**
  **The mechanism is missing, not mis-tuned:** volume has no price and scarcity has no reward.
  Measured — usage 29.1→45.2% while FG% *rises* 44.5→45.8 (no cost to being a ball-hog); and a
  thirty-point usage drop (43%→13%) buys **2.8pp** of shooting, ~0.1pp per usage point where real
  basketball is 3–5× that (no reward for a reduced role). **Two of Emmett's players cannot exist:**
  the star who goes past 40% but pays "a pretty hefty efficiency hit," and the underathletic sniper
  who is efficient *because* he's the 4th option. Same flat curve, opposite ends. The Phase 17
  usage diet shift is meant to be this price and is not biting.
  **Emmett's anchor ruling (2026-07-14): the ~31.7% at 43% usage is plausibly CORRECT — it is the
  LOW-usage end that is wrong.** So the fix **anchors the top and lifts the bottom**; it is not a
  re-tune of the star. **Spec:** usage >~40% unrealistic, <~8–10% unrealistic (`UsageFloor` 0.09
  already matches); extreme usage permitted *only* with a real efficiency cost.
  **Why this and not Pass B:** Pass B's entire payoff is "the denied drive becomes *someone else's
  better shot*" — the exact wire proven dead here; built on a flat curve it ships as a measured
  no-op. And the season page cannot calibrate anything until it's fixed: today every star is railed
  at 45% and every role player floored at 13% regardless of who they are, so the page would lie to
  a generation tune of 21,000 players.
  **Not blocked by Emmett's stop ruling** — that correctly stops the *magnitude* chase (how steep?
  what exactly is a #1's usage?), which needs a real population. A **missing mechanism** gets
  designed the way the drive gate was: Python oracle → archetype table → Emmett's ruling on the
  SHAPE → placeholders → the page tunes later. **The archetype table is the sign-off medium**, e.g.
  *"your sniper as 5th option on a good team: 3P 37% → 42%; your creator forced to 45%: FG 32% →
  27%."*
  **SCOPE WALL (Emmett, 2026-07-14): the curve only.** `HierarchyRank`'s coarseness and the
  rail-authors-usage finding are parked below — findings, not this session's mandate.
  Full §6 pipeline; oracle-first (the core is a formula); the prompt is its own audited pass, NOT
  drafted from S59.2's momentum (CONVENTIONS §6).
