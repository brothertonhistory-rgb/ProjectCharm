...
- **Attribute-meaning layer — the `sweep` bench is BUILT (S45), its picker bug FIXED (S46), and its
  ...
  **height-over-defender make term the item that most gates the height→skill lean sizing**. **That term
  ...
  the wiring work it depends on — and the height→skill lean now waits only on the queued S55 sweep
  re-measurement, not on a missing wire.
- **The S50 physical-package design questions — (1) is RULED AND SHIPPED (S55); (2)–(4) remain open.**
  ...
  gated now waits only on the queued sweep re-measurement (Height/Wingspan walks diffed vs the saved
  S50 curves — the wire exists, the new curve is unmeasured). (2) **The block channel is
