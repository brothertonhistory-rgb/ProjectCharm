...
- **Attribute-meaning layer — the `sweep` bench is BUILT (S45), its picker bug FIXED (S46), and its
  ...
  *blame* (committer picker) but not *protection* (the halfcourt matchup is pressure-gated to zero
  ...
  channel). Full findings in `docs/attribute-meaning.md`, Family E. The remaining families, each
  following the same template: **A. Body** (Height, Wingspan,
  Weight) · **C. Athleticism** (7 ratings) · **F. Interior offense** (PostMoves, Screening) ·
