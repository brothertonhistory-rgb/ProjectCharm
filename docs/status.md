...
- **Attribute-meaning layer — the `sweep` bench is BUILT (S45), its picker bug FIXED (S46), and its
  ...
  **height-over-defender make term the item that most gates the height→skill lean sizing**. **That term
  ...
  track:** (1) **the ball-dominance/initiation layer** (the S49 reshaper — the engine models who shoots
  but not who initiates/holds the ball; unlocks real assist volume and handle-protection; the next
  design conversation), (2) ~~the unforced-turnover channel (S49)~~ — **SHIPPED S56** (handling now moves the neutral turnover base; flattens the inversion, full flip deferred — see below), (3) ~~the no-post-hunt diet gap (S51)~~ — **SHIPPED S57** (PostMoves now tilts the interior diet + resists displacement + discounts interior assists; see below),
  (4) the S54 defensive findings (perimeter-defense point-neutrality; Discipline near-inert — a
  candidate wiring gap), (5) the remaining S50 physical questions (block channel light; wingspan→steals
  unwired; Weight cosmetic). The pressure-dialed channels (BallHandling, Steals turnover-forcing) stay
...
- **Two engine reshapers surfaced by the S49 perimeter-creation measurement — design conversations,
  ...
  design conversation of the attribute-meaning pass. (2) **The unforced-turnover channel — SHIPPED
  ...
  **flipping** it. The attribution sub-call named at S49 — lean unforced blame toward the *weaker*
  ...
  usage, correct for steals, backwards for dead-balls); it lands with the per-event touch/initiation
  attribution model, and would let the full inversion flip.
