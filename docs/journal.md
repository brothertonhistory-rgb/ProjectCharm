...
**One honest scope note.** The instruments captured this run are the frozen-corpus observation (one fixed non-representative roster pair) and the 20-school fixture calibration — **not** the 347-school `stock-d1` season page proper (the "FTA 15.8" instrument). The direction and magnitude are decisive, but the exact 347-page FTA/FT-rate line is recorded here as *expected on target, pending its own invocation*. **Residual read for bridge #3:** FTA reached target from the bonus fix alone, so the parked shooting-foul-*rate* bump, the `FoulDrawing` diagnosis, and late-game intentional fouling stay parked and are **no longer urgent** — the whistle rate did not need raising to close the gap; the missing fouls were the ones the tracker was silently dropping.
...
**Next:** with FTA closed by the bonus fix, the remaining season-page gaps are the open field — **rebounds-LOW / steals-LOW** (the credited-rebound / live-ball reconciliation) and **assists-HIGH / turnovers-HIGH / blocks-HIGH** (likely probabilistic-attribution artifacts more than real over-production), plus **OT-LOW**. **Player-generation Pass 2** (tweener-post existence, weakest-leg development) remains bridged and ready. Pick the gap before drafting; the next-session prompt is its own audited pass. The 347-school season page should be run once to record the exact post-S40 FTA/FT-rate line.
