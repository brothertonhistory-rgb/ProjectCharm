...
public sealed class Player
{
    ...
    /// <summary>Latent (ceiling) skill ratings by skill key — what this player can
    /// grow into. Empty until a Pass-2-generated population is wired in (Phase 3+).</summary>
    public IReadOnlyDictionary<string, int> LatentSkills { get; init; } = EmptySkillMap;

    /// <summary>Current (expressed) skill ratings by skill key — the Pass-2 source of
    /// the authored skill card. Empty until Phase 3+ wiring.</summary>
    public IReadOnlyDictionary<string, int> CurrentSkills { get; init; } = EmptySkillMap;

    /// <summary>Latent minus current, by skill key — the development runway the future
    /// development layer spends. Empty until Phase 3+ wiring.</summary>
    public IReadOnlyDictionary<string, int> Runway { get; init; } = EmptySkillMap;

    /// <summary>Arrival stage in [0,1]: how much of the latent card is already
    /// expressed (0 = raw project, 1 = fully arrived). The RULED mechanism behind
    /// class/age (S42.1). Defaults to 0.0 — unread until Phase 3+.</summary>
    public double Arrival { get; init; }

    /// <summary>Class label (Fr/So/Jr/Sr) — PLACEHOLDER decoration on Arrival per the
    /// S42.1 ruling; the season layer owns the real population-structure question.
    /// Empty string = not generated by the Pass-2 path.</summary>
    public string PlayerClass { get; init; } = "";

    private static readonly IReadOnlyDictionary<string, int> EmptySkillMap =
